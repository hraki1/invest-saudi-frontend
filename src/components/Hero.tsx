      ...
      <div>
        ...
        <div className="relative z-10 container mx-auto px-6 md:px-12 lg:px-20 mb-20">
          <div className="max-w-2xl">
            <h1 className="text-3xl bukra-bold sm:text-4xl md:text-5xl lg:text-6xl font-extralight text-white mb-6 md:mb-8 leading-snug tracking-wide">
              {t("hero.title")}
            </h1>

            <p className="text-sm sm:text-[22px] bukra-regular text-white/90 mb-8 md:mb-12 leading-relaxed">
              {t("hero.subtitle")}
            </p>
          </div>

          {/* actions */}
          <div className="flex flex-col justify-between  sm:flex-row flex-wrap gap-4">
            {/* search input */}
            <div className="min-w-[300px] sm:min-w-sm relative mb-5 sm:mb-0">
              <input
                type="text"
                placeholder="i am looking for.."
                className="min-w-full sm:min-w-sm px-4 py-3.5 rounded-xl bg-black/30 text-white backdrop-blur-2xl border border-white/30 placeholder:text-white/70"
              />
            ...
            <div className="flex flex-col bukra-regular sm:flex-row flex-wrap gap-4">
              {actions.map((key) => (
                <button
                  key={key}
                  className="flex justify-between gap-3 items-center px-7 py-3.5 rounded-4xl bg-transparent border border-white/80 text-white"
                >
                  {t(key)}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-5 gap-5">
            {stats.map((stat, index) => {
              ...
              return (
                <div dir="ltr"
                  key={index}
                  className="rounded-lg p-5 flex items-center md:gap-5"
                >

        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative flex items-center justify-center">
            <svg viewBox="0 0 400 400" className="w-full max-w-lg">
              ...
              {[...Array(12)].map((_, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    />
                    {/* Small decorative icon in each segment */}
                    <circle
                      cx={200 + (innerRadius + outerRadius) / 2 * Math.cos((angle + 12.5) * Math.PI / 180)}
                      cy={200 + (innerRadius + outerRadius) / 2 * Math.sin((angle + 12.5) * Math.PI / 180)}
                      r="8"
                      fill="white"
                      opacity="0.9"
                    />
              ...
              {/* Highlighted Segment - changes based on active section */}
              <path
                d="M 200 80 L 200 20 A 180 180 0 0 1 280 52 L 240 104 A 120 120 0 0 0 200 80"
                fill={sectionContent[activeSection].highlightColor}
                stroke="white"
                strokeWidth="3"
              ...
              {/* Center Circle */}
              <circle cx="200" cy="200" r="115" fill="white" stroke="#e5e7eb" strokeWidth="2"/>
              
              {/* Center Text - Dynamic based on active section */}
              {sectionContent[activeSection].percentage ? (
                <>
                  <text x="200" y="180" textAnchor="middle" className="text-5xl font-light" fill={sectionContent[activeSection].highlightColor}>
                    {sectionContent[activeSection].percentage}
                  </text>
                  <text x="200" y="210" textAnchor="middle" className="text-sm font-light" fill="#6b7280">
                    {sectionContent[activeSection].centerText}
                  </text>
                  {sectionContent[activeSection].centerSubtext.map((text, idx) => (
                    <text key={idx} x="200" y={230 + (idx * 20)} textAnchor="middle" className="text-sm font-light" fill="#6b7280">
                      {text}
                    </text>
                  ))}
                </>
                ...
                <>
                  <text x="200" y="190" textAnchor="middle" className="text-4xl font-light" fill={sectionContent[activeSection].highlightColor}>
                    {sectionContent[activeSection].centerText}
                  </text>
                  {sectionContent[activeSection].centerSubtext.map((text, idx) => (
                    <text key={idx} x="200" y={220 + (idx * 20)} textAnchor="middle" className="text-sm font-light" fill="#6b7280">
                      {text}
                    </text>
                  ))}
                </>
              ...
              {/* Arrow indicators around the circle */}
              {[0, 60, 120, 180, 240, 300].map((angle, i) => {
                const arrowAngle = (angle - 90) * (Math.PI / 180);
                const x = 200 + 190 * Math.cos(arrowAngle);
                const y = 200 + 190 * Math.sin(arrowAngle);
                
                return (
                  <polygon
                    key={i}
                    points={`${x},${y-8} ${x+8},${y} ${x},${y+8}`}
